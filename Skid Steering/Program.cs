    ...
    partial class Program : MyGridProgram
    {
        ...
        */
        ...
        void SkidSteer()
        {
            ...
            ApplyGyroOverride(0, isTurning ? _gyroRotationSpeedRpm * MathHelper.RPMToRadiansPerSecond * gyroTurnDirection : 0, 0, _gyros, controller.WorldMatrix, gyroPower);
            ...
            bool isTurning)
        {
            foreach (var w in wheels)
            {
                ...
                Vector3D referenceForward = reference.WorldMatrix.Forward;
                Vector3D gridForward = reference.CubeGrid.WorldMatrix.Forward;

                double dotProduct = Vector3D.Dot(referenceForward, gridForward);

                float propulsion = 0f;

                if (dotProduct > 0.99f) // or close to 1.0f
                {
                    propulsion = leftPropulsion;
                }
                else if (dotProduct < -0.99f) // or close to -1.0f
                {
                    propulsion = rightPropulsion;
                }
                ...
                if (moveInPlace)
                {
                    w.PropulsionOverride = (isLeft ? leftPropulsion : rightPropulsion) * w.Power * 0.01f;
                }
                else w.PropulsionOverride = sign * propulsion * w.Power * 0.01f;
...
new AnimationParams(WasdKey.A, 15f),
        ...
        ass rotation axes. 
        */
        void ApplyGyroOverride(Vector3D rotationSpeedPYR, List<IMyGyro> gyros, MatrixD worldMatrix, float gyroPower)
        {
            ...
            foreach (var g in gyros)
            {
                ...
                g.GyroOverride = gyroPower == 0? false : true;
            }
        }

        void ApplyGyroOverride(double pitchSpeed, double yawSpeed, double rollSpeed, List<IMyGyro> gyros, MatrixD worldMatrix, float gyroPower)
        {
            var rotationVec = new Vector3D(pitchSpeed, yawSpeed, rollSpeed);
            ApplyGyroOverride(rotationVec, gyros, worldMatrix, gyroPower);
        }
